...
searchInputBox.addEventListener('keypress', (event) => {
    if (event.keyCode == 13) {
        // Add an event listener to the input box to listen for the 'Enter' key press
        getWeatherReport(searchInputBox.value);
        
    }
})
// Function to fetch the weather report for the given city
function getWeatherReport(city) {
    fetch(`${weatherApi.baseUrl}?q=${city}&appid=${weatherApi.key}&units=metric`) 
        .then(weather => { 
            return weather.json();
        }).then(showWeaterReport); // Display the weather report

}
// Function to display the weather report
function showWeaterReport(weather) {
    let city_code=weather.cod;
    // Handle empty input and invalid city name cases
    if(city_code==='400'){ 
...
// Function to get the appropriate icon class for the weather status
